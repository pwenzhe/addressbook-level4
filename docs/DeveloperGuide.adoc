...
This guide will be useful even if you are already an experienced contributor.
...
The calendar feature allows user to store birthdays. Users can further make use of this feature to sort the database.
This feature is supported by visual where user can see their friend's name on the calendar.
This feature would require changes on all component of the architecture.

==== Logic

===== Add Command
The date variable can be added via the add command as this is one of the possible data that an be stored by the user.
Minor changes has been changed to AddCommand and AddCommandPaser. The changes made ensures that this additional variable follows the format of the initial four varaible.

===== Find Command
User can categorise people based on their birthday and the database will be sorted based on the day when user is viewing it.
This is implemented using the find command as this will reduce the number of commands used by the users.



==== Model

Date is added the same way as the other variables for person.
->diagram
As can be seen from the diagram, date is accessed the same way as the other four variables.
...
==== UI
A calendar will be added. It will be a monthly view such that users can see their friend's name appearing on the calendar.
When the user select a person, the calendar will bring them to the upcoming birthday date for that person.
...
Similarly, the GoogleMapBrowserPanel will display the map of the selected person's home location using either the postal code or the address in the person details. The UiParts is responsible for constructing and displaying
...
=== Delete Multiple Persons Mechanism

The Delete mechanism is able to support deleting multiple persons. Refer to diagram 2.3.1 for the basic implementation of the
Delete mechanism.

At `DeleteCommandParser`, the index ranges will be parsed into a list of `IndexRange` class using the `ParserUtil`, where each `IndexRange`
object contains the start and end index of each range. This list will then be converted into a `HashSet` of indexes, which
will be used to construct a `DeleteCommand`. `HashSet` prevents the duplicates of indexes. Upon executing the `DeleteCommand`,
a sub list of person to delete will be produced to delete the information in `Model`.
