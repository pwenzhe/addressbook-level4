...
image::BirthdayStatisticsCommandSequenceDiagram.png[width="400"]
_Figure 4.6.2: Birthday Statistics Command Sequence Diagram_
...
image::InstagramBrowserPanelSequenceDiagram.png[width="400"]
...
The home command will show users the home panel of the app. This panel displays the total number of
...
The help command will show users the help panel of the app. This panel displays a summarised table of
...
image::HomeCommandSequenceDiagram.png[width="400"]
...
as the parameter of the event.
. `MainApp` will handle the new event and push to the `Ui`, which will push to the `MainWindow`.
. In `MainWindow` class, `changeInformationPanel()` will perform the panel change depending on the
parameter in the event.
. `Home` will then return `CommandResult` with `MESSAGE_SUCCESS` as the parameter back to the `LogicManager`.
// end::homeHelpCommand[]
...
birthday statistics and tag statistics command are the current commands
that will post new `ChangeInformationPanelRequestEvent(panel)` event.
...
. Following the flow of the diagram, `MainApp` will handle the new event and push to the `Ui`, which will push to the `MainWindow`.
. In `MainWindow` class, `changeInformationPanel()` will perform the panel change depending on the
parameter in the event.
...
**Aspect:** Type of Information Panel +
...
display. +
**Cons:** Extra implementation required for individual commands that will require changing of the panel. +
...
The suggestions are preloaded using keywords from the features in Bevy.
...
Bevy can export data out onto a .csv file when users enter the export command. Seen in the figure above is the actual file Bevy exports the data to.

image::ExportCommandSequenceDiagram.png[width="400"]
_Figure 4.12.2: Export Command Sequence Diagram_

image::SaveToCsvFileSequenceDiagram.png[width="400"]
...
. `MainApp` will handle the new event, as seen in figure 4.12.3 and save the current addressbook data using `StorageManager`.
. `ExportCommand` will then return `CommandResult` with `MESSAGE_SUCCESS` as the parameter back to the `LogicManager`.
