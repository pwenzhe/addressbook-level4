...
. Command: `find m/Feb` +
Expected Result: Returns all persons that have their birthday in the month of `Feb`.
. Command: `f bedok favourite` +
Expected Result: Returns all persons that has their address in `Bedok` and all persons pinned as favourite.
